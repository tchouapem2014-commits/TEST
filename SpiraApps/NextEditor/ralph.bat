@echo off
REM ============================================================
REM RALPH WIGGUM - NextEditor Development Loop v1.0
REM ============================================================
...
echo ============================================================
echo  RALPH WIGGUM - NextEditor Development
echo ============================================================
...
if exist nexteditor.js (
    echo [RALPH] Checking JavaScript syntax...
    node -c nexteditor.js >nul 2>&1
    if !errorlevel! neq 0 (
...
if !errorlevel! equ 0 (
    ...
    if exist package.json (
        call npm run build --input="%CD%\..\NextEditor" 2>&1
        if !errorlevel! equ 0 (
            echo [RALPH] Package generated successfully!
            for %%f in (*.spiraapp) do (
                move /Y "%%f" "..\NextEditor\" >nul 2>&1
                echo [RALPH] Moved %%f to NextEditor folder
            )
        ) else (
